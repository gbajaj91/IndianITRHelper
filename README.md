...
Use source mode `etrade_benefit_history` (the default) for ESPP/RSU grants and vests, or
`etrade_holdings_bystatus` for a simpler "Sellable" holdings snapshot.
...
## Run the script
With the virtual environment activated, run the script with the downloaded export file:
```sh
# ETRADE (default source mode)
./run.py -i "<absolute_folder>/BenefitHistory.xlsx" -ay 2023

# Charles Schwab
./run.py -i "<absolute_folder>/Individual_..._Transactions_....csv" -m schwab_transactions -ay 2023
```

Detailed options are listed below
```txt
usage: run.py [-h] [-o OUTPUT_FOLDER] -i INPUT_EXCEL_FILE
              [-m {etrade_benefit_history,etrade_holdings_bystatus,schwab_transactions}]
              [-cal {calendar,financial}] [-r {fa,capital_gains}] -ay
              ASSESSMENT_YEAR [-v] [--skip-refresh]
...
options:
  ...
  -i INPUT_EXCEL_FILE, --input INPUT_EXCEL_FILE
                        ...
                        (`BenefitHistory.xlsx`) Excel file for etrade source modes, or the
                        transactions CSV export for `schwab_transactions`
  -m {etrade_benefit_history,etrade_holdings_bystatus,schwab_transactions}, --source-mode {etrade_benefit_history,etrade_holdings_bystatus,schwab_transactions}
...
# Limitations
- `BenefitHistory.xlsx` (etrade) and the Schwab transactions CSV are the only supported input formats.
- Sell handling differs by source: Schwab sells are matched FIFO against buys and reduce the
  reported holding. ETRADE RSU releases have no sell tracking at all (the RSU sheet doesn't record
  sales), and ESPP purchases only know a lot was fully sold as of some date, not with enough detail
  for the capital gains report - see the Capital gains report limitation above.
